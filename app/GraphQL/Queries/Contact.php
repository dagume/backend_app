...
(select con.folder_id, doc_con.created_at, doc_con.updated_at, doc_con.doc_id, doc_ref.drive_id
            ...
            right join (select doc_rol.id, req_doc.name_required_documents, doc_rol.role_id as role_id from documents_rol as doc_rol
            inner join required_documents as req_doc
            on doc_rol.required_document_id = req_doc.id where doc_rol.role_id in (select role_id from members where contact_id = ? ))
            as req_doc
            on req_doc.id = doc_upload.doc_id', [$args['contact_id'], $args['contact_id']]);
